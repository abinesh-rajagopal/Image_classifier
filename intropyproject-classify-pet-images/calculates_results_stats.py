...
def calculates_results_stats(results_dic):
    ...
    for image_file_name, image_result in results_dic.items():
        n_any_pet_match += image_result[2]
        n_dogs_img += image_result[3]
        n_match += image_result[2]
        #number of correctly classified dog images
        if image_result[3] == 1 and image_result[4] == 1:
            n_correct_dogs += 1
            if image_result[2] == 1:
                n_correct_breed += 1

        #number of correctly classified NON-dog images
        if image_result[3] == 0 and image_result[4] == 0:
            n_correct_notdogs += 1
    ...
    #n_notdogs_img - number of NON-dog images
    results_stats_dic['n_notdogs_img'] = total_images - n_dogs_img
    #n_match - number of matches between pet & classifier labels
    ...
    #pct_match - percentage of correct matches
    results_stats_dic['pct_match'] = (n_any_pet_match / total_images ) * 100
    #pct_correct_dogs - percentage of correctly classified dogs
    ...
    #pct_correct_notdogs - percentage of correctly classified NON-dogs
    results_stats_dic['pct_correct_notdogs'] = (n_correct_notdogs / (total_images - n_dogs_img )) * 100

    #Incorrectly Classified Dogs
    results_stats_dic['incorrect_dogs'] = n_dogs_img - n_correct_dogs

    results_stats_dic['incorrect_breed'] = n_dogs_img - n_correct_breed
